...
export async function getBankTransfers(walletAddress: string): Promise<BankTransferRecord[]> {
...
export async function submitBankTransfer(data: BankTransferSubmission): Promise<BankTransferResult> {
  ...
  if (!response.ok) {
    ...
    throw new Error((err as any).message || `Failed to submit bank transfer: ${response.statusText}`);
  }

  const result = await response.json() as any;
  return {
    transferId: result.data?.transferId || result.transferId || result.data?.data?.transferId,
    status: result.data?.status || result.status || 'Pending',
    message: result.message || 'Bank transfer submitted successfully',
  };
}

async function uploadProofFile(file: File, walletAddress: string): Promise<string> {
